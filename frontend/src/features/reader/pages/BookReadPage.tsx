...
function useBookRead(bookId: string | undefined) {
  ...
  const load = useCallback(async () => {
    ...
    // 선택된 표지(selected 우선, 없으면 candidate 첫 번째)
    ...
    await loadComments(chaptersData, chapterIds)
  }, [bookId])
...
export default function BookReadPage() {
  ...
  const displayName: string =
    user?.user_metadata?.full_name ?? user?.user_metadata?.name ?? profile?.display_name ?? '나'
  ...
  async function handleSubmitComment() {
    if (!commentText.trim() || !activeChapterId || !profile || !book) return
    setSubmitting(true)
    try {
      ...
      await supabase.from('notifications').insert({
        ...
        type: 'new_comment',
        title: `${displayName}이 댓글을 남겼어요`,
        body: commentText.trim(),
        reference_id: activeChapterId,
        reference_type: 'chapter',
      })

      setCommentText('')
      showToast('댓글을 전달했어요')
      // Realtime이 reload를 처리하므로 직접 reload 불필요 (fallback으로 남겨둠)
      await reload()
    } catch {
      ...
      <main className="flex-1 overflow-y-auto w-full max-w-2xl mx-auto">
        ...
        {chapters.length === 0 ? (
          ...
          <>
                ...
                >
                  ...
                  <div className="relative flex flex-col items-center gap-3">
                    ...
                    <p className="text-base text-white/70">
                      {seniorName} 지음 · {book.year}년 {book.month}월
                    </p>
                  </div>
            ...
            {/* 댓글 섹션 */}
            {activeChapter && (
              <div className="bg-white mx-3 mt-3 mb-4 rounded-2xl px-5 py-5 flex flex-col gap-4">
