...
- [x] 개발 환경 구성 (로컬 Supabase CLI, Deno Edge Function)
...
- [x] **[F-01]** Supabase 마이그레이션 작성
  - [x] 13개 테이블 생성 (`profiles`, `senior_profiles`, `family_links`, `memories`, `conversations`, `utterances`, `books`, `chapters`, `comments`, `replies`, `cover_images`, `notifications`, `book_generation_jobs`)
  - [x] RLS 정책 전체 적용
  - [x] Storage 버킷 3개 생성 (`avatars`, `book-covers`, `reply-audio`)
  - [x] `pg_cron` 월말 트리거 설정
  - [x] Realtime publication 설정
  - [x] `supabase/seed.sql` 목업 데이터 작성 (어르신 1명·가족 3명·책 3권·챕터 12개·댓글·알림)
- [x] **[F-09]** Whisper 파인튜닝 시작
  - [x] AI Hub 107·94 데이터셋 다운로드 · 전처리 스크립트 작성
  - [x] Colab Pro+ 환경 세팅 · LoRA 파인튜닝 1차 시작 (전처리 진행 중)
...
- [x] **[F-03]** AI 말동무 음성대화 — 1차 (기본 연결) ✅ 2026-04-27 완료
  ...
  - [x] 말동무 화면 기본 UI (`src/features/chat/`)
