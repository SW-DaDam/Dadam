...
| 플랜 검증 | **Codex** | 플랜의 로직 검토, 엣지 케이스·리스크 점검, 수정 의견 |
| 코드 작성 | **Claude Code** | 검증된 플랜 기반으로 기능 구현, 리팩터, 문서 작성 |

- **운영 규약**
  - Claude가 플랜을 작성하면, **코드 작성 전에** Codex가 플랜을 검증합니다.
  - Codex 의견은 사용자를 통해 전달되며, 반영 여부는 작성자가 판단합니다.
  - Codex 검증이 완료된 뒤 Claude가 코드를 작성합니다.
