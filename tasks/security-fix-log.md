...
> 인접 관찰(미수정, 별도 판단 필요): `job_id` 처리 경로에서 인증된 사용자가 **타인의 pending job_id**를 넘겨도 소유권을 재검증하지 않음(IDOR 가능성). P1-1 범위 밖이라 미반영 — 사용자 결정 대기.
...
| 항목 | 상태 |
|------|------|
| P2-4 net_http_post_cover SSRF | ✅ 수정(마이그레이션) |
| P2-5 notifications INSERT | ⏸️ 결정 대기 (가족 서클) |
| P2-6 chapter-photos 경로 소유권 | ✅ 수정(마이그레이션) |
| P2-7 reply-audio 읽기 | ⏸️ 결정 대기 (P2-5 결합) |
| P2-8 반응 이모지 정합 | ✅ 수정(마이그레이션) |
| P2-9 book 생성 원자성 | ⏸️ 결정 대기 (회귀 위험) |
| P2-10 replies RLS book_id | ✅ 오탐(수정 불필요) |

**수정 3 · 오탐 1 · 결정 대기 3**. 결정 대기 3건(P2-5/7/9)은 사용자 선택 후 진행.

> **결정 반영(이후)**: P2-5=SECURITY DEFINER 헬퍼, P2-7=SECURITY DEFINER 헬퍼, P2-9=타깃 보강 B. 모두 ✅ 완료(위 각 항목 참조). **P2 7건 전부 처리 완료**.
